...
In this section, we describe our experimental methodology for evaluating the SO(3) tube smoothing algorithm. We design experiments to validate both the theoretical guarantees and practical performance on both synthetic and real-world datasets.
...
\begin{itemize}
\item \textit{Tube Constraint Satisfaction:} Maximum and average constraint violation:
\begin{equation}
\text{violation}_{\text{max}} = \max_{i=0}^{N-1} \left( \left\| \log(R_i^T \hat{R}_i) \right\|_2 - \epsilon_i \right)
\end{equation}
\begin{equation}
\text{violation}_{\text{avg}} = \frac{1}{N} \sum_{i=0}^{N-1} \max\left( \left\| \log(R_i^T \hat{R}_i) \right\|_2 - \epsilon_i, 0 \right)
\end{equation}
...
We design synthetic experiments to validate theoretical properties and demonstrate feasibility guarantees.
...
We evaluate:
\begin{itemize}
\item \textbf{Constraint satisfaction:} Maximum violation across all samples
...
\end{equation}
This provides optimal smoothness but no feasibility guarantees.
